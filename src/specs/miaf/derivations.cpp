...
{
  static const std::initializer_list<RuleDesc> rulesDerivations = {
    { "Section 7.3.9\n"
    ...
    { "Section 7.3.11.1\n"
      ...
      [](Box const &root, IReport *out) {
        ...
        auto check = [&](const std::list<uint32_t> &visited) {
          ...
          for(int i = 0, expectIdx = 0; i < numDerivations && expectIdx < (int)expected.size();
              ++i, ++visitedIt, ++expectIdx) {
      ...
      [](Box const &root, IReport *out) {
        ...
        for(auto srcItemId : sourceItemIds) {
          ...
          if(!isVisualSampleEntry(fourcc))
            out->error(
              "All derivation chains shall originate from one or more coded images: itemID=%u doesn't "
              "(type=%s)",
              srcItemId, type);
        }
      } },
