...
{
  static const std::initializer_list<RuleDesc> rulesAudio = {
    { "Section 7.4.5\n"
                                  ...
                                  if(stsdChild.fourcc == FOURCC("twos"))
                                    for(auto &sym : stsdChild.syms)
                                      if(!strcmp(sym.name, "channelcount"))
                                        if(sym.value > 2)
                                          out->error(
                                            "Uncompressed audio of type 'twos' "
                                            "found shall be mono or "
                                            "stereo, found %d channels",
                                            sym.value);
      } },
    { "Section 7.4.5\n"
                                  ...
                                  if(stsdChild.fourcc == FOURCC("mp4a"))
                                    for(auto &sym : stsdChild.syms)
                                      if(!strcmp(sym.name, "channelcount"))
                                        if(sym.value > 2)
                                          out->error(
                                            "channelcount shall not exceed 2, "
                                            "found %lld",
                                            sym.value);
      } },
    { "Section 7.4.5\n"
                                    ...
                                    for(auto &sym : stsdChild.syms)
                                      if(!strcmp(sym.name, "samplerate"))
                                        if((sym.value >> 16) > 48000)
                                          out->error(
                                            "Sampling rate shall not exceed "
                                            "48000, found %lld",
                                            sym.value >> 16);
      } },
  };
