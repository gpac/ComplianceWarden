  ...
  for(auto &fromItem : d.itemRefs) {
      ...
      if(numPixels > lastNumPixels) // "next larger"
      {
        if(lastNumPixels != 0) {
          ...
          if(numPixels > 200 * lastNumPixels)
            out->error(
              "Next larger \"%s\" has %d times more pixels (resolutions: %llu vs %dx%d). Limit is 200",
              toString(fourcc).c_str(), numPixels / lastNumPixels, lastNumPixels, d.itemRes[toItem].width,
              d.itemRes[toItem].height);
        }
    ...
    if(!foundLargerItem) {
      ...
      if((lastNumPixels != 0) && (numPixels > 200 * lastNumPixels))
        out->error(
          "Next larger is a master image that has %d times more pixels (resolutions: %llu vs %dx%d). Limit is "
          "200",
...
{
  static const std::initializer_list<RuleDesc> rulesNumPixels = {
    { "Section 7.3.3\n"
      ...
      "item",
      [](Box const &root, IReport *out) { validateNextLarger(root, out, FOURCC("thmb")); } },
    { "Section 7.3.11.4.2\n"
